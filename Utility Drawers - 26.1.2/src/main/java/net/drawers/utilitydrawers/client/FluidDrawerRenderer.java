    ...
    public void submit(FluidDrawerRenderState state, PoseStack poseStack, SubmitNodeCollector submitNodeCollector, CameraRenderState cameraRenderState) {
        ...
        try {
            ...
            for (int i = 0; i < slotCount; i++) {
                ...
                float scale = fluidScales[i];

                FluidStack stack = state.fluids[i];

                net.minecraft.world.level.block.state.BlockState fluidState = stack.getFluid().defaultFluidState().createLegacyBlock();

                // 1. Fixed Model Dispatch Resolution: Query via modelManager's blockStateModelSet
                ...
                // 2. Fixed Tint Evaluation: Look up the source registry manually and evaluate tint at layer 0
                int tint = -1;
                ...
                int fluidColor = tint == -1
                        ? (stack.getFluid() == net.minecraft.world.level.material.Fluids.WATER ? 0xFF3F76E4 : 0xFFFFFFFF)
                        : (0xFF000000 | tint);
                ...
                submitNodeCollector.submitCustomGeometry(poseStack, renderType, (pose, consumer) -> {
                    ...
                    consumer.addVertex(matrix, -0.5f, -0.5f, 0.0f).setColor(fluidColor).setUv(sprite.getU0(), sprite.getV1()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(15728880).setNormal(pose, 0.0f, 0.0f, 1.0f);
                    consumer.addVertex(matrix,  0.5f, -0.5f, 0.0f).setColor(fluidColor).setUv(sprite.getU1(), sprite.getV1()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(15728880).setNormal(pose, 0.0f, 0.0f, 1.0f);
                    consumer.addVertex(matrix,  0.5f,  0.5f, 0.0f).setColor(fluidColor).setUv(sprite.getU1(), sprite.getV0()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(15728880).setNormal(pose, 0.0f, 0.0f, 1.0f);
                    consumer.addVertex(matrix, -0.5f,  0.5f, 0.0f).setColor(fluidColor).setUv(sprite.getU0(), sprite.getV0()).setOverlay(OverlayTexture.NO_OVERLAY).setLight(15728880).setNormal(pose, 0.0f, 0.0f, 1.0f);
                });
                poseStack.popPose();
                ...
                poseStack.translate(ox, textOy - (scale * 0.3f), 0.4376D);
